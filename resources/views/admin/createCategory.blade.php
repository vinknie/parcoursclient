...
<div class="">
    ...
    <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
        class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
        <span class="block sm:inline">{!! \Session::get('error3') !!}</span>
        <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
            <svg class="fill-current h-6 w-6 text-red-500" role="button" xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20">
                <title>Close</title>
    ...
    @endif
    <form class="bg-white p-6 rounded-lg" action="{{ url('/dashboard/createCategory/createDialogue') }}" method="post"
        enctype="multipart/form-data">
        @csrf
        <div class="mb-4">
            <label class="labels block text-gray-700 font-medium mb-2">Etape du dialogue</label>
            <select name="">
                <option value="">--Selectionner l'étape--</option>
                @foreach ($getCategory as $category)
                <option value="{{ $category->id_category }}"> {{ $category->title }} </option>
        ...
        <div class="mb-4">
            <label class="labels block text-gray-700 font-medium mb-2">Verbatim du dialogue</label>
            <select name="id_verbatim">
